...
import { mockSales, mockCustomers } from "@/lib/mock-data";
import { PageHeader, MetricCard } from "@/components/shared/page-header";
import { CREDIT_LIMIT } from "@/types";
import { AlertTriangle, Download, BookOpen, Users } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
function getCustomerBalance(customerId: number) {
  const sales = mockSales.filter((s) => s.customer_id === customerId);
  const totalBill = sales.reduce(
    (sum, s) => sum + s.quantity * s.rate_per_bag + s.rickshaw_fare,
    0
  );
  const totalCashPaid = sales.reduce((sum, s) => sum + s.cash_received, 0);
  return {
    total_bill: totalBill,
    total_cash_paid: totalCashPaid,
    total_goods_value: 0,
    balance_due: totalBill - totalCashPaid,
  };
}
...
export default function CustomerKhataPage() {
  ...
  const selectedCustomer = useMemo(
    () => mockCustomers.find((c) => c.id === Number(selectedCustomerId)),
    [selectedCustomerId]
  );
  ...
  const selectedSales = useMemo(() => {
    ...
    const sales = mockSales.filter(
      (s) => s.customer_id === Number(selectedCustomerId)
    );
    return sales;
  }, [selectedCustomerId]);
  ...
  const groupedSales = useMemo(() => {
    const groupMap = new Map<string, typeof selectedSales>();
    ...
    groupMap.forEach((_, key) => {
      billMap.set(billNum, key);
      billNum++;
    });

    // Also create a reverse lookup: saleId -> billNumber
    const saleToBill = new Map<number, number>();
    let bn = 1;
    groupMap.forEach((sales, _key) => {
      sales.forEach((s) => {
        saleToBill.set(s.id, bn);
      });
      bn++;
    });

    // Sort groups by first sale created_at
    const sortedGroups: {
      billNumber: number;
      groupId: string;
      sales: typeof selectedSales;
    }[] = [];
    billMap.forEach((groupId, billNumber) => {
      sortedGroups.push({
        billNumber,
        groupId,
        sales: groupMap.get(groupId)!,
      });
    });

    return { sortedGroups, saleToBill };
  }, [selectedSales]);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        variant={
                          c.type === "credit" ? "default" : "secondary"
                        }
                        className="text-xs"
                      >
                        {c.type}
          ...
          <div className="p-4 sm:p-6 space-y-6">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      <th className="text-left text-xs uppercase text-slate-500 font-semibold px-3 py-2.5">
                        Bill #
                      </th>
                      <th className="text-left text-xs uppercase text-slate-500 font-semibold px-3 py-2.5">
                        Date
                      ...
                      <th className="text-right text-xs uppercase text-slate-500 font-semibold px-3 py-2.5">
                        Qty
                      </th>
                      <th className="text-right text-xs uppercase text-slate-500 font-semibold px-3 py-2.5">
                        Rate
                  ...
                  <tbody>
                    {groupedSales.sortedGroups.map((group) =>
                      group.sales.map((sale, idx) => {
                        const billAmount =
                          sale.quantity * sale.rate_per_bag + sale.rickshaw_fare;
                        const unitLabel =
                          sale.unit_type === "kg" ? "kg" : "bags";
                        return (
                          <tr
                            key={sale.id}
                            className={cn(
                              ...
                              idx === group.sales.length - 1 &&
                                group !==
                                  groupedSales.sortedGroups[
                                    groupedSales.sortedGroups.length - 1
                                  ] &&
                                "border-b-slate-200"
                            )}
                          >
                            {idx === 0 && (
                              <td
                                className="px-3 py-2.5 font-bold text-slate-700 align-top"
                                rowSpan={group.sales.length}
                              >
                                #{group.billNumber}
                              </td>
                            )}
                            {idx === 0 && (
                              <td
                                className="px-3 py-2.5 text-slate-600 align-top"
                                rowSpan={group.sales.length}
                              >
                                {sale.sale_date}
                              </td>
                            ...
                            <td className="px-3 py-2.5 text-right tabular-nums">
                              {sale.rickshaw_fare > 0
                                ? fmt(sale.rickshaw_fare)
                                : "—"}
                            </td>
            ...
            {selectedBalance && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard
                  label="Total Billed"
                  value={fmt(selectedBalance.total_bill)}
                  color="blue"
                  prefix="Rs. "
                />
                <MetricCard
                  label="Cash Paid"
                  value={fmt(selectedBalance.total_cash_paid)}
                  color="green"
                  prefix="Rs. "
                />
                <MetricCard
                  label="Paid in Goods"
                  value={fmt(selectedBalance.total_goods_value)}
                  color="purple"
                  prefix="Rs. "
                />
                <MetricCard
                  label="Balance Due"
                  value={fmt(selectedBalance.balance_due)}
                  color="orange"
                  prefix="Rs. "
                />
              </div>
