...
// Suppliers change rarely — 30s TTL
const SUPPLIERS_TTL = 30_000;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const suppliers = await cachedGet(
      userKey(auth.user.id, "suppliers", suffix),
      [userTag(auth.user.id, "suppliers")],
      SUPPLIERS_TTL,
      () => getAllSuppliers(activeOnly),
    );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    invalidateByTag(userTag(auth.user.id, "suppliers"));
