...
export async function GET() {
  ...
  for (const fn of RPC_FUNCTIONS) {
    try {
      // Try a zero-argument call with wrong types to see if the function exists
      // We use a generic approach: try to call with dummy params and check error type
      const { error } = await admin.rpc(fn);
      // If the function exists, we'll get a parameter count/type error, not "does not exist"
      if (error) {
