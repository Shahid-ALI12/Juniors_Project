...
-- 3. record_purchase
-- p_location_id is now the LAST parameter and defaults to NULL.
-- Stock is ALWAYS incremented (locations concept removed).
-- Handles both 'bags' and 'kg' unit purchases (kg → bags conversion).
...
-- Now allows NULL location_id (locations concept removed)
...
BEGIN
  ...
  -- Decrement (location_id can be NULL now — use IS NOT DISTINCT FROM)
